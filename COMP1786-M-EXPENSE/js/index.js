...
function confirmTrip(e) {
    ...
    $('#page-create #frm-confirm #location').text($('#page-create #frm-register #location').val());
    $('#page-create #frm-confirm #note').text($('#page-create #frm-register #note').val());
    $('#page-create #frm-confirm #owner').text($('#page-create #frm-register #owner').val());
...
function registerTrip(e) {
    ...
    let location =$('#page-create #frm-register #location').val();
    let note =$('#page-create #frm-register #note').val();
    let owner =$('#page-create #frm-register #owner').val();

    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name, Destination, Description, Location ,Owner ,Note, Risk, Date) VALUES (?, ?, ?, ?, ?, ?, ?, julianday('${date}'))`;
        tx.executeSql(query, [name, destination, description,location, owner,note, risk], transactionSuccess, transactionError);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $('#page-detail #detail #location').text(result.rows[0].Location);
                $('#page-detail #detail #note').text(result.rows[0].Note);
                $('#page-detail #detail #owner').text(result.rows[0].Owner);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            ...
            if (result.rows[0] != null) {
                ...
                $(`#page-detail #frm-update #owner`).val(result.rows[0].Owner);
                $(`#page-detail #frm-update #location`).val(result.rows[0].Location);
...
function updateTrip(e) {
    ...
    let location = $('#page-detail #frm-update #location').val();
    let owner = $('#page-detail #frm-update #owner').val();
    let note = $('#page-detail #frm-update #note').val();

    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?, Destination = ?, Description = ?,Location = ?,Owner = ?,Note = ?, Risk = ?, Date = julianday('${date}') WHERE Id = ?`;

        tx.executeSql(query, [ name, destination, description,location, owner,note, risk,id], transactionSuccess, transactionError);
